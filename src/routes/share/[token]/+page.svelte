...
<svelte:head><title>{data.name} · Shared tree · HeritageAtlas</title></svelte:head>
...
{#if !data.unlocked}
	...
	<div class="mx-auto mt-12 flex max-w-sm flex-col gap-4">
		<div class="text-center">
			...
			<p class="mt-1 text-sm text-ink/60">This shared family tree is password-protected.</p>
		</div>
...
{:else if data.view && data.tree}
	<div class="flex flex-col gap-3">
		<div class="flex flex-col gap-1 text-center">
			...
			<p class="text-sm text-ink/55">Shared, read-only view · photos hidden for privacy</p>
		</div>
