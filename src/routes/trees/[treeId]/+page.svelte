<script lang="ts">
	...
	onMount(() => {
		...
		if (imported) {
			toasts.success(t('tree.view.importedPeople', { count: Number(imported) }));
...
<svelte:head><title>{t('tree.view.title', { name: data.tree.name })}</title></svelte:head>

{#if !hasPeople}
	<div class="grid flex-1 place-items-center p-8 text-center">
		<div class="flex max-w-sm flex-col items-center gap-4">
			<p class="text-ink/55">
				{t('tree.view.nopeople', { name: data.tree.name })}
			</p>
			{#if data.canEdit}
				<a
					href={addPersonUrl}
...
{:else}
	...
	<div class="relative flex min-h-0 flex-1">
		...
		<div class="absolute top-2 left-2 z-30 flex items-center gap-2">
			...
			>
				{showNotes ? t('tree.view.notesOn') : t('tree.view.notesOff')}
			</button>
		</div>
